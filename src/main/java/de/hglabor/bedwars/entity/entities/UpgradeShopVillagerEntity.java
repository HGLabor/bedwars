...
public class UpgradeShopVillagerEntity extends GameEntity<Entity> {
    ...
    public UpgradeShopVillagerEntity(Location location) {
        super(Localization.getMessage("map.entities.upgradeshop.name", Locale.ENGLISH), (Class<Entity>) list.get(new Random().nextInt(list.size()-1)), location);
    }

    @Override
    public void onClick(Entity context, Player player) {
        //TODO open upgrade shop
    }
}
